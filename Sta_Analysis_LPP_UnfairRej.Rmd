---
title: "LPP Emotion Comparison in Unfair-Reject Condition - Complete Analysis"
author: "Jin Gao"
date: "`r Sys.Date()`"
...
output_dir <- "export/offer_lpp_accept_reject/Unfair_Reject_Emotion_Analysis"
...
cat("===== EMOTION VALIDATION IN UNFAIR CONDITION =====\n")
unfair_emotion_check <- data_analysis %>%
...
cat("Unfair condition emotion × response distribution:\n")
print(unfair_emotion_check)
cat("\n")

# Specifically check Unfair-Reject
unfair_reject_check <- data_analysis %>%
  filter(offer_type == "Unfair" & response == "Reject") %>%
  group_by(emotion) %>%
  summarise(
    n = n(),
...
cat("Unfair-Reject specific check:\n")
print(unfair_reject_check)

# Check for data anomalies
if(length(unique(round(unfair_reject_check$var, 2))) == 1) {
...
trial_counts_wide <- trial_counts %>%
  dplyr::select(-n_subjects) %>%  # Fixed: added dplyr::
  pivot_wider(
...
# Main Analysis: Unfair-Reject Emotion Comparison

```{r unfair-reject-analysis}
cat("\n===== UNFAIR-REJECT EMOTION COMPARISON (LPP) =====\n\n")

# Filter for Unfair-Reject condition only
data_unfair_reject <- data_analysis %>%
  filter(offer_type == "Unfair" & response == "Reject")

cat("Unfair-Reject trials:", nrow(data_unfair_reject), "\n")
cat("Participants with Unfair-Reject trials:", n_distinct(data_unfair_reject$participant_id), "\n\n")

# Descriptive statistics by emotion
desc_stats <- data_unfair_reject %>%
  group_by(emotion) %>%
  summarise(
    n_trials = n(),
    n_subjects = n_distinct(participant_id),
...
cat("Descriptive Statistics (Unfair-Reject):\n")
print(as.data.frame(desc_stats))
write.csv(desc_stats, file.path(output_dir, "descriptive_stats_unfair_reject.csv"), row.names = FALSE)
```
...
model_full <- tryCatch({
  lmer(LPP ~ emotion + (emotion|participant_id), 
       data = data_unfair_reject, REML = FALSE)
...
boxplot(residuals(model_full) ~ data_unfair_reject$emotion,
...
# Define emotion colors and labels as per your specification
emotion_colors <- c(
  'neu' = '#C5C5C5EC',
  'aff' = '#39E04F', 
  'dis' = '#755627',
...
p1 <- ggplot(desc_stats, aes(x = emotion, y = mean_LPP, fill = emotion)) +
  ...
  ylim(0, 2.5) +  # Adjusted y-axis for better visibility
  labs(title = "LPP Amplitude by Emotion (Unfair-Reject Condition)",
       subtitle = paste0("Total trials: ", nrow(data_unfair_reject), 
                        " | Participants: ", n_distinct(data_unfair_reject$participant_id),
...
p2 <- ggplot(context_summary, aes(x = emotion, y = mean_LPP, 
  ...
  scale_x_discrete(labels = emotion_labels) +
  coord_cartesian(ylim = c(-2, 3.5)) +  # Adjusted for context comparison
  labs(title = "LPP Across All Conditions (Context)",
       subtitle = "Unfair-Reject (red) is the focus of main analysis",
       x = "Emotion", 
       y = "LPP Amplitude (μV)",
       fill = "Condition") +
  theme_minimal(base_size = 12) +
...
if(p_value < 0.05) {
  subject_means <- data_unfair_reject %>%
  ...
  p3 <- ggplot(subject_means, aes(x = emotion, y = mean_LPP)) +
    ...
    scale_x_discrete(labels = emotion_labels) +
    labs(title = "Individual Subject Patterns (Unfair-Reject)",
         subtitle = "Gray lines = individuals | Red = grand mean",
         x = "Emotion", 
         y = "LPP Amplitude (μV)") +
    theme_minimal(base_size = 12) +
    theme(plot.title = element_text(face = "bold"),
...
# Combine plots (now only 3 plots)
library(gridExtra)
if(p_value < 0.05) {
...
# Save plots
ggsave(file.path(output_dir, "LPP_emotion_plots.png"), 
       combined_plot, width = 14, height = 10, dpi = 300)

# Save individual plots
ggsave(file.path(output_dir, "LPP_barplot.png"), p1, width = 8, height = 6, dpi = 300)
ggsave(file.path(output_dir, "LPP_context.png"), p2, width = 10, height = 6, dpi = 300)
if(p_value < 0.05) {
  ggsave(file.path(output_dir, "LPP_individual_trajectories.png"), p3, width = 8, height = 6, dpi = 300)
}
...
subject_emotion_counts <- data_unfair_reject %>%
  group_by(participant_id, emotion) %>%
...
cat("ANALYSIS: LPP amplitude comparison across 5 emotions in Unfair-Reject condition\n")
cat("=========================================================================\n\n")
...
cat("DATA SUMMARY:\n")
cat("- Total Unfair-Reject trials analyzed:", nrow(data_unfair_reject), "\n")
cat("- Number of participants:", n_distinct(data_unfair_reject$participant_id), "\n")
...
cat("- Visualization plots (bar, violin, context, individual)\n")

# Save workspace
save.image(file.path(output_dir, "LPP_Unfair_Reject_Emotion_Analysis.RData"))
cat("\nWorkspace saved.\n")
cat("\n========== ANALYSIS COMPLETE ==========\n")
```